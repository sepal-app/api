...
def upgrade():
    ...
    op.drop_table('role')
    op.add_column('organization_user', sa.Column('created_at', sa.DateTime(), server_default=sa.text("TIMEZONE('utc', CURRENT_TIMESTAMP)"), nullable=False))
    op.add_column('organization_user', sa.Column('role', sa.String(), nullable=False))
    op.add_column('organization_user', sa.Column('updated_at', sa.DateTime(), server_default=sa.text("TIMEZONE('utc', CURRENT_TIMESTAMP)"), nullable=False))
...
def downgrade():
    ...
    sa.ForeignKeyConstraint(['organization_id'], ['organization.id'], name='role_organization_id_fkey'),
    sa.PrimaryKeyConstraint('id', name='role_pkey'),
    sa.UniqueConstraint('organization_id', 'name', name='role_organization_id_name_key'),
    ...
    sa.ForeignKeyConstraint(['role_id'], ['role.id'], name='role_member_role_id_fkey'),
    sa.PrimaryKeyConstraint('id', name='role_member_pkey'),
    sa.UniqueConstraint('role_id', 'user_id', name='role_member_role_id_user_id_key')
    )
    ...
    sa.Column('organization_id', sa.INTEGER(), autoincrement=False, nullable=False),
    sa.Column('name', sa.VARCHAR(), autoincrement=False, nullable=False),
    sa.ForeignKeyConstraint(['organization_id'], ['organization.id'], name='user_permission_organization_id_fkey'),
    sa.PrimaryKeyConstraint('id', name='user_permission_pkey'),
    sa.UniqueConstraint('user_id', 'organization_id', 'name', name='user_permission_user_id_organization_id_name_key')
    )
