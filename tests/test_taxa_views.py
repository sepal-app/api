...
def test_taxa_list(client, auth_header, org, taxon):
    resp = client.get(f"/orgs/{org.id}/taxa", headers=auth_header)
...
def test_create(client, auth_header, make_token, org):
    ...
    resp = client.post(f"/orgs/{org.id}/taxa", headers=auth_header, json=data)
...
def test_taxon_detail(client, auth_header, org, taxon):
    resp = client.get(f"/orgs/{org.id}/taxa/{taxon.id}", headers=auth_header)
...
def test_taxon_detail_missing(client, auth_header, org, taxon):
    ...
    resp = client.get(f"/orgs/{org.id}/{other_taxon_id}", headers=auth_header)
